...
/**
 ...
 deprectated use sof_cdb_put_control(SOF_CDB_MESSAGELEVEL and SOF_CDB_RETURNTYPE instead
...
/**
 ...
 deprectated use sof_cdb_put_control(SOF_CDB_LOCK_HANDLING instead
...
enum CDB_CONTROL
{
   SOF_CDB_INIT_STATUS = 1,
   SOF_CDB_CTRL_LANGUE,
   SOF_CDB_ABORT_ON_ERROR,
   SOF_CDB_LAST_ERROR_CODE,
   SOF_CDB_LAST_ERROR_CODE_EXISTS,
   SOF_CDB_MEMORY_SIZE_BLOCK,   // size_t
   SOF_CDB_PRECACHE,
   SOF_CDB_LOCK_DEFAULT,
   SOF_CDB_LOCK_HANDLING,
   SOF_CDB_DEFAULT_VERSION,
   SOF_CDB_MESSAGELEVEL,
   SOF_CDB_RETURNTYPE,
   SOF_CDB_AUTO_MEMORY_RESIZE,  // 1 to enable, 0 = off default
   SOF_CDB_LOCK_ENCOUNTERED,
};

SOF_C_DLL_API void sof_cdb_put_control(CDB_CONTROL ctrl, int val);
SOF_C_DLL_API void sof_cdb_put_control(CDB_CONTROL ctrl, size_t val);
SOF_C_DLL_API int sof_cdb_get_control(int ctrl);

SOF_C_DLL_API void sof_cdb_put_control_str(int ctrl, const std::string&);
#define SOF_CDB_CTRL_TEMP_FOLDER 100
...
*/
SOF_C_DLL_API std::string sof_cdb_getname(int index);

/**
 \ingroup cdbase   CDBASE

 \param name  kompletter dateiname utf8 encoded
...
SOF_C_DLL_API int sof_cdb_isopen(const std::string& name);
SOF_C_DLL_API int sof_cdb_isopen(const std::wstring& name);
/**
 \ingroup cdbase   CDBASE

 die Datei wird geoeffnet und automatisch aktuelle Datenbank

 \param filename vollstaendiger Dateiname utf8 encoded
 ...
 \param  nid = CD_OPEN_EXISTING   Datei nur aufmachen, wenn sie existiert
...
SOF_C_DLL_API int sof_cdb_init_string(const std::string& filename, int nid);
SOF_C_DLL_API int sof_cdb_init_wstring(const std::wstring& filename, int nid);
...
#define CD_OPEN_EXISTING 93
#define CD_OPEN_REMOVE 92
...
SOF_C_DLL_API cd_err_type sof_cdb_peek(int index, int kwh, int kwl, void* data, int* len, int pos);
...
*/
SOF_C_DLL_API void cd_Util_FreeForce();
SOF_C_DLL_API void cd_Util_FreeForce(int index);



SOF_C_DLL_API void sof_cdb_reset_stats(int index);
SOF_C_DLL_API std::string sof_cdb_get_stats(int index);
...
namespace sof { namespace cdbase {
   class SOF_C_DLL_API Listener
   {
   public:
      Listener();
      virtual ~Listener();

      virtual void opened(int /*cdb*/)
      {
      }
      virtual void closed(int /*cdb*/)
      {
      }
      virtual void flushed(int /*cdb*/)
      {
      }
      virtual void kw_was_written(int /*index*/, int /*kwh*/, int /*kwl*/)
      {
      }
      virtual void kw_was_deleted(int /*index*/, int /*kwh*/, int /*kwl*/)
      {
      }
   };
   ...
   public:
      explicit ResetUsage() {}
      explicit ResetUsage(int new_index) { sof_cdb_use(new_index, 0); }
      ~ResetUsage() { sof_cdb_use(m_oldIndex, 0); }

   private:
      int m_oldIndex = sof_cdb_status(CD_STATUSQ_AKTIV);
   };

   class LockFileResetter
   {
   public:
      explicit LockFileResetter(int index) : m_index(index) { m_cd_error = sof_cdb_use(m_index, 1); }
      ~LockFileResetter() { sof_cdb_use(m_index, -1); }
      bool lockIsSet() { return m_cd_error == CD_ERR_NONE; }

   private:
      int m_index;
      cd_error_t m_cd_error = CD_ERR_NONE;
   };

   class LockHandlingResetter
   {
   public:
      explicit LockHandlingResetter(int newHandling) { sof_cdb_put_control(SOF_CDB_LOCK_HANDLING, newHandling); }
      ~LockHandlingResetter() { sof_cdb_put_control(SOF_CDB_LOCK_HANDLING, m_oldLockHandling); }

   private:
      int m_oldLockHandling = sof_cdb_get_control(SOF_CDB_LOCK_HANDLING);
   };

}} // namespace sof::cdbase

// Export a selected subset of the CDBASE API as extern "C" so that we can use them via FFI (e.g. in SOFiPLUS LuaJIT FFI).
extern "C" {
   SOF_C_DLL_API int c_api_sof_cdb_init(const char* filename, int nid);
   SOF_C_DLL_API cd_err_type c_api_sof_cdb_put(int index, int kwh, int kwl, const void* s, const int* ls, int nrew);
   SOF_C_DLL_API void c_api_sof_cdb_close(int Index);
   SOF_C_DLL_API void c_api_sof_cdb_free_ex(int index, int kwh, int kwl);
   SOF_C_DLL_API cd_err_type c_api_sof_cdb_get(int index, int kwh, int kwl, void* s, int* ls, int nrew);
   SOF_C_DLL_API cd_err_type c_api_sof_cdb_rewind_ex(int index, int kwh, int kwl);
   SOF_C_DLL_API cd_err_type c_api_sof_cdb_enq(int index, int* kwh, int* kwl, int isk);
   SOF_C_DLL_API void c_api_sof_cdb_kenq_ex(int index, int* kwh, int* kwl, int isk);
}
