...
class CDESC_KW(Structure):         # -999/-997  description string to group
   _fields_ = [
         ('m_kwh', c_int),         #        Kwh
         ('m_kwl', c_int),         #        Kwl
...
class CCTRL_OBJ(Structure):        # 0/01:101  involved objects
   _fields_ = [
         ...
         ('m_box', c_float * 3 * 2)  # [1001] bounding box x,y,z min, x,y,z max
      ]
cctrl_obj = CCTRL_OBJ()
...
class CMAT_CONC(Structure):        # 1/NR:1  MaterialConcrete
   _fields_ = [
         ...
         ('m_ec', c_float),        #        compression failure Energy (deprecated)
...
class CMAT_STEE(Structure):        # 1/NR:1  MaterialSteel
   _fields_ = [
         ...
         ('m_fyc', c_float),       # [1092] compression yield stress
         ('m_ftc', c_float),       # [1092] compression strength
         ('m_tmax', c_float),      # [1025] maximum plate thickness
...
class CMAT_TIMB(Structure):        # 1/NR:1  MaterialTimber
   _fields_ = [
         ...
         ('m_fyt90', c_float),     # [1092] tensile strength perpendicular the fibres
         ('m_fyc', c_float),       # [1092] compressive strength along the fibres
         ('m_fyc90', c_float),     # [1092] compressive strength perpendicular the fibres
...
class CMAT_BRIC(Structure):        # 1/NR:1  MaterialBrickwork
   _fields_ = [
         ...
         ('m_fbt', c_float),       # [1092] tensile brick strength
...
class CMAT_SERV(Structure):        # 1/NR:2  StressStrainLaw (Servicability)
   _fields_ = [
         ...
         ('m_epst', c_float),      # [1081] Elongation at TEMP if TEMP>0
         ('m_epss', c_float),      # [1081] Shift of strains TEMP>0 (-TempStrain)
         ('m_fctm', c_float),      #        Tensile Strength  Concrete   for tension stiffening
         ('m_arb', c_float * 5 * 20)  #        Function values
      ]
cmat_serv = CMAT_SERV()
...
class CMAT_BED(Structure):         # 1/NR:7  MaterialBedding
   _fields_ = [
         ...
         ('m_dil', c_float),       #        dilatancy coefficient
...
class CMAT_CRE(Structure):         # 1/NR:90  ExplicitCreepCurve
   _fields_ = [
         ...
         ('m_rval', c_float * 2 * 16)  #        real material values (time[d],value pairs)
      ]
cmat_cre = CMAT_CRE()
...
class CMAT_SHR(Structure):         # 1/NR:90  ExplicitShrinkageCurve
   _fields_ = [
         ...
         ('m_title', c_int),       #        title of the curve
         ('m_rval', c_float * 2 * 16)  # [1081] real material values (time[d],value pairs),
      ]
cmat_shr = CMAT_SHR()
...
class CBORE(Structure):            # 2/NR:0  SoilProfile
   _fields_ = [
         ...
         ('m_alf', c_float),       # [1200] Angle of reference
...
class CBORE_LAY(Structure):        # 2/NR:1  Soillayer
   _fields_ = [
         ...
         ('m_phi', c_float),       # [1200] Soil/Pile friction angle
         ('m_gam', c_float),       # [1091] nominal weight
...
class CAXIS_VIZ(Structure):        # 3/ID:98  Data points for visualisation
   _fields_ = [
         ...
         ('m_pts', c_float * 3 * 64)  # [1001] Data Points
      ]
caxis_viz = CAXIS_VIZ()
...
class CAXIS_ATB(Structure):        # 3/ID:101  Geometric segments of axis plan view
   _fields_ = [
         ...
         ('m_tau', c_float),       # [1200] deflection angle
...
class CAXIS_ITB(Structure):        # 3/ID:103  Placement at axis position (obsolete)
   _fields_ = [
         ...
         ('m_alf', c_float),       # [1200] Rotation about global Z
         ('m_alfx', c_float),      # [1200] Rotation about local axis x
         ('m_alfy', c_float),      # [1200] Rotation about transverse y
         ('m_alfz', c_float),      # [1200] Rotation about vertical   z
         ('m_incr', c_float),      # [1200] Inclination to the right +y
         ('m_incl', c_float),      # [1200] Inclination to the left  -y
...
class CAXIS_ITM(Structure):        # 3/ID:10104  Secondary axis parameter mapping
   _fields_ = [
         ...
         ('m_ids', c_int),         #        Enum of secondary axis (1='A' to 26 ='Z')
         ('m_map', c_float * 2 * 64)  #        Mapping S (Main axis) --> S (secondary axis)
      ]
caxis_itm = CAXIS_ITM()
...
class CGAXD_ATB(Structure):        # 3/ID:121  Geometric segments of axis plan view (double precision)
   _fields_ = [
         ...
         ('m_tau', c_double),      # [1200] deflection angle
...
class CAXIS_TRL(Structure):        # 3/ID:300  Properties for train loading
   _fields_ = [
         ...
         ('m_inca', c_float),      # [1200] Inclination within lane at SA
         ('m_ince', c_float),      # [1200] Inclination within lane at SE
         ...
         ('m_asl', c_float)        # [1001] Distance of Sleepers or mounting points (not effective currentl
      ]
caxis_trl = CAXIS_TRL()
...
class CAREA(Structure):            # 5/ID:0  Geometric Area
   _fields_ = [
         ...
         ('m_t', c_float * 3 * 4), #        Transformation
         ('m_par', c_float * 16),  #        additional Parameters
         ('m_box', c_float * 2 * 3), # [1001] boundig box xmin,xmax,ymin,ymax,zmin,zmax
         ('m_text',  c_int * 17)   #        Designation of Line
      ]
carea = CAREA()
...
class CAREA_CPI(Structure):        # 5/ID:12  COONsPatchInfo
   _fields_ = [
         ...
         ('m_sp', c_float * 3 * 4),#        Length parameters for the edges iu0,iu1,iv0,iv1
         ('m_uv', c_float * 2 * 2),#        umin,umax,vmin,vmax of the parameter plane
         ('m_cxyz', c_float * 3 * 4), #        Coordinates of the corner points C1,C2,C3,C4
         ('m_tinc', c_float * 3 * 8), #        inclinations for a bicubic patch as vectors
         ('m_twist', c_float * 3 * 4)  #        Twist of x,y,z-Ordinate at corner C1 (0,0)
      ]
carea_cpi = CAREA_CPI()
...
class CCON_MEM(Structure):         # 8/ID:10  Connected Member data
   _fields_ = [
         ...
         ('m_roty', c_float),      # [1200] Angle of end plate y-rotation
         ('m_rotz', c_float),      # [1200] Angle of end plate z-rotation
         ...
         ('m_voute', c_float * 2 * 4)  # [1011] Voute [lenght,height] for
      ]
ccon_mem = CCON_MEM()
...
class CCON_PLT(Structure):         # 8/ID:20  Additional plates in connection
   _fields_ = [
         ...
         ('m_t', c_float),         # [1011] Thickness of plate
         ('m_edge', c_float * 5 * 8)  # [1011] Local coordinates, weldid, weldthickness
      ]
ccon_plt = CCON_PLT()
...
class CSSCT_CS(Structure):         # 1009/NR:-  Construction stage start in sub data
   _fields_ = [
         ('m_id', c_int),          #        Identification
         ('m_mno', c_int),         #        Materialnumber of sections
         ('m_mrf', c_int),         #        Materialnumber of Reinforcement
...
class CSECT_TRA(Structure):        # 9/NR:8  SectionalAppliedTransformation
   _fields_ = [
         ...
         ('m_ival', c_int),        #        type of transformation such as IVAL from USR_ITP_TRANS
...
class CSECT_PRO(Structure):        # 9/NR:12  Standard rolled steel shapes (PROF)
   _fields_ = [
         ...
         ('m_alf', c_float),       # [1200] Rotation angle
         ('m_it', c_float),        # [1014] Tabulated torsional inertia
         ('m_ysc', c_float),       # [1011] Tabulated shear center y
         ('m_zsc', c_float),       # [1011] Tabulated shear center z
         ('m_h', c_float),         # [1011] Height
         ('m_b', c_float),         # [1011] Width
         ('m_s', c_float),         # [1011] Thickness of web
         ('m_t', c_float),         # [1011] Thickness of flange
         ('m_r1', c_float),        # [1011] First radius
         ('m_r2', c_float),        # [1011] Second radius
         ('m_yoff', c_float),      # [1011] Y coordinate of offset
         ('m_zoff', c_float),      # [1011] Z coordinate of offset
         ('m_cm', c_float),        # [1016] Tabulated warping resistance
         ('m_wmin', c_float),      # [1012] Characteristic warping 1
         ('m_wmax', c_float),      # [1012] Characteristic warping 2
         ('m_wt', c_float),        # [1018] Torsional resistance
         ('m_ay', c_float),        # [1012] Shear area Ay
         ('m_az', c_float),        # [1012] Shear area Az
         ...
         ('m_yref', c_float),      # [1011] Y coordinates of reference point
         ('m_zref', c_float),      # [1011] Z coordinates of reference point
         ('m_b2', c_float),        # [1011] Width lower part
         ('m_t2', c_float),        # [1011] Lower flange thickness
         ('m_bez', c_int * 8),     #        Character of shape
         ('m_vypl', c_float),      # [1102] Characteristic plastic shear force Vy       !Charakteristische 
         ('m_vzpl', c_float),      # [1102] Characteristic plastic shear force Vz       !Charakteristische 
         ('m_mtpl', c_float),      # [1103] Characteristic plastic torsional moment Mt  !Charakteristische 
         ('m_mt2pl', c_float),     # [1103] Characteristic plastic torsional moment Mt2 !Charakteristische 
         ('m_mbpl', c_float)       # [1103] Characteristic plastic warping moment Mb    !Charakteristische 
      ]
csect_pro = CSECT_PRO()
...
class CSECT_CW(Structure):         # 9/NR:80  Hydrodynamic coefficients (WIND/Wave loading)
   _fields_ = [
         ...
         ('m_ag', c_float),        # [1200] angle of attack in degrees
...
class CSECT_WPA(Structure):        # 9/NR:81  SectionWindParameters for Wind loading (WPAR)
   _fields_ = [
         ...
         ('m_alf', c_float)        # [1200] rotation for default wind coefficients
      ]
csect_wpa = CSECT_WPA()
...
class CSECT_WDE(Structure):        # 9/NR:82  SectionWindDerivativa for Wind loading (WIND)
   _fields_ = [
         ...
         ('m_der', c_float * 5 * 20)  #        Derivativa vred,value as splines
      ]
csect_wde = CSECT_WDE()
...
class CSECT_USR(Structure):        # 9/NR:90  SectionUserValues (SV/QW)
   _fields_ = [
         ...
         ('m_ival', c_int),        #        Integer value of control flag
         ('m_rval', c_float * 32)  #        Real value of control flag
      ]
csect_usr = CSECT_USR()
...
class CSSCT_SPT(Structure):        # 1009/NR:100  SectionFemPoint
   _fields_ = [
         ('m_id', c_int),          #        Identification 100/101
         ('m_mno', c_int),         #        Material number
         ('m_idp', c_int),         #        Number*256 + Bitpattern
...
class CSSCT_PPT(Structure):        # 1009/NR:101  SectionFemPolygonPoint
   _fields_ = [
         ('m_id', c_int),          #        Identification 100/101
         ('m_mno', c_int),         #        Material number
         ('m_idp', c_int),         #        Number*256 + Bitpattern
...
class CSSCT_PTC(Structure):        # 1009/NR:310  PartialCutIntegrationPoint of shear cut
   _fields_ = [
         ('m_id', c_int),          #        Identification 100/101
         ('m_mno', c_int),         #        Material number
         ('m_idp', c_int),         #        Number*256 + Bitpattern
...
class CSSCT_SNZ(Structure):        # 1009/NR:111  SectionNeffZones
   _fields_ = [
         ('m_id', c_int),          #        Identification 111
         ('m_neffzone', c_int),    #        Id of noneffectivity per construction stage
         ('m_id2', c_int),         #        free
         ('m_tnr', c_int),         #        Designation
         ('m_csneff', c_int * 33)  #        Construction stages effectivity
      ]
cssct_snz = CSSCT_SNZ()

class CSSCT_FEM(Structure):        # 1009/NR:112  SectionFemTopology
   _fields_ = [
         ('m_id', c_int),          #        Identification 112
         ('m_mno', c_int),         #        Material number
         ('m_idp', c_int),         #        Properties
         ('m_nr', c_int),          #        Element number
         ('m_grp', c_int),         #        Group number (start of construction stage)
         ('m_neffzone', c_int),    #        Id of noneffectivity per construction stage
         ('m_node', c_int * 4),    #        Node numbers
         ('m_y', c_float),         # [1011] y-ordinate mid point
         ('m_z', c_float)          # [1011] z-ordinate mid point
      ]
cssct_fem = CSSCT_FEM()

class CSSCT_NOD(Structure):        # 1009/NR:113  SectionFemNode
   _fields_ = [
         ('m_id', c_int),          #        Identification 113
         ('m_id1', c_int),         #        free
         ('m_idp', c_int),         #        Properties
...
class CSECT_NEP(Structure):        # 9/NR:191  Partial non effective polygons
   _fields_ = [
         ...
         ('m_yz', c_float * 2 * 511)  # [1011] yz(2,*) = vertices (old format is blocked with 7 points
      ]
csect_nep = CSECT_NEP()
...
class CSSCT_PCP(Structure):        # 1009/NR:320  SectionCutPointReinforcement for partial cut polygon
   _fields_ = [
         ('m_id', c_int),          #        Identification 200
         ('m_mno', c_int),         #        Material number
         ('m_ir', c_int),          #        Layer number
...
class CSSCT_LCP(Structure):        # 1009/NR:321  SectionCutLineReinforcement for partial cut polygon
   _fields_ = [
         ('m_id', c_int),          #        Identification 201/211/221 or 321
         ('m_mno', c_int),         #        Material number
         ('m_ir', c_int),          #        Layer number
...
class CSECT_CPL(Structure):        # 9/NR:311  Partial polygons of shear cut
   _fields_ = [
         ...
         ('m_yz', c_float * 2 * 511)  # [1011] yz(2,*) = up to seven vertices
      ]
csect_cpl = CSECT_CPL()
...
class CSECT_SUB(Structure):        # 9/NR:1009  SectionalSubDataBlock
   _fields_ = [
         ('m_id', c_int),          #        identifier 1009
         ('m_block', c_int),       #        consecutive number of the block
         ('m_type', c_int),        #        Type of following items
         ('m_tnr', c_int),         #        Designation of the block
         ('m_nrec', c_int)         #        Number of following records
      ]
csect_sub = CSECT_SUB()

class CSSCT_SUB(Structure):        # 1009/NR:1009  SectionalSubDataBlock
   _fields_ = [
         ('m_id', c_int),          #        identifier 1009
         ('m_block', c_int),       #        consecutive number of the block
         ('m_type', c_int),        #        Type of following items
...
class CSYST(Structure):            # 10/00  SystemInfo
   _fields_ = [
         ...
         ('m_igres', c_int),
         ('m_box', c_float * 3 * 2), # [1001] bounding box
         ('m_tg', c_float * 3 * 4),#        global CDB-System transformation matrix
...
class CSYST_C_A(Structure):        # 10/1:3  Actions for Possible Combination of actions
   _fields_ = [
         ('m_id', c_int),          #        identifier = 3
         ('m_act_grp', c_int),     #        Identifier of action or action group
         ('m_faku', c_float),      #        unfavourable factor
         ('m_fakf', c_float),      #        favourable factor
         ('m_act', c_int * 30)     #        Identifier of action for action group
      ]
csyst_c_a = CSYST_C_A()
...
class CLC_MACRO(Structure):        # 12/LC:600  Marks begin/end of macro generated loads
   _fields_ = [
         ('m_id', c_int),          #        identifier 600
         ('m_tag', c_int),         #        Macro tag
         ('m_orig_typ', c_int),    #        Type of load generating macro
         ('m_orig_id', c_int)      #        Id of load generating macro
      ]
clc_macro = CLC_MACRO()
...
class CLC_WIND(Structure):         # 12/LC:400  wind loading
   _fields_ = [
         ...
         ('m_lat', c_float),       # [1200] explicit lattitude of site
...
class CACT_TASK(Structure):        # 14/0  Current superposition task commands
   _fields_ = [
         ...
         ('m_uform', c_int * 32)   #        User defined target function (index == 0!)
      ]
cact_task = CACT_TASK()
...
class CLC_ACT1(Structure):         # 14/NR:1  Action member
   _fields_ = [
         ...
         ('m_action', c_int),      #        > 0 ID of the action = type of action, see default definition 0
...
class CCSG_CS(Structure):          # 15/-10  CSG
   _fields_ = [
...
class CCSG_CGRP(Structure):        # 15/-11  CSG
   _fields_ = [
...
class CCSG_CLAS(Structure):        # 15/-12  CSG
   _fields_ = [
         ('m_ics', c_int),         #        corresponding cs number
...
class CCSG_CSYS(Structure):        # 15/-13  CSG
   _fields_ = [
         ('m_ics', c_int),         #        corresponding cs number
...
class CCSG_CCRL(Structure):        # 15/-14  CSG
   _fields_ = [
         ('m_ics', c_int),         #        corresponding cs number
...
class CCSG_CGW(Structure):         # 15/-15  CSG
   _fields_ = [
         ('m_ics', c_int),         #        corresponding cs number
...
class CCSG_ECHO(Structure):        # 15/-16  CSG
   _fields_ = [
         ('m_ics', c_int),         #        corresponding cs number
...
class CCSM_CSCR(Structure):        # 15/-19  CSM Creep construction stages header record
   _fields_ = [
         ('m_lc_diff', c_int),     #        negative base load case number for incremental responses LDIFF
         ('m_nsets', c_int),       #        total # of sets = max(# materials, # groups)
         ('m_ncrp', c_int),        #        total # of creep stages
         ('m_cs_crp', c_int * 9999)  #        CS numbers of creep stages, dim = NCRP
      ]
ccsm_cscr = CCSM_CSCR()

class CCSM_DPHI(Structure):        # 15/-19  CSM Creep coefficient increments for creep stages by group and 
   _fields_ = [
         ('m_grp', c_int),         #        group number
         ('m_mno', c_int),         #        material number
         ('m_it0', c_int),         #        load application time counter IT0 = [1, NCRP]
         ('m_dphis', c_float * 9999)  # [1299] Creep coefficient increments d_phi(ti,t0) per creep step ti and
      ]
ccsm_dphi = CCSM_DPHI()

class CCSM_CSSH(Structure):        # 15/-20  CSM Shrinkage construction stages header record
   _fields_ = [
         ('m_lc_diff', c_int),     #        negative base load case number for incremental responses LDIFF
         ('m_nsets', c_int),       #        total # of sets = max(# materials, # groups)
         ('m_ncrp', c_int),        #        total # of creep stages
...
class CCSM_DEPS(Structure):        # 15/-20  CSM Shrinkage strain increments for stages by group and materia
   _fields_ = [
         ('m_grp', c_int),         #        group number
         ('m_mno', c_int),         #        material number
         ('m_it0', c_int),         #        load application time counter IT0 = [1, NCRP]
         ('m_depss', c_float * 9999)  # [1299] Shrinkage strain increments per creep step ti and load
      ]
ccsm_deps = CCSM_DEPS()
...
class CVIEW(Structure):            # 18/-1  View definitions (Exchange ANIMATOR-WINGRAF)
   _fields_ = [
         ...
         ('m_fovy', c_float),      # [1200] perspective angle of field of view
...
class CN_MPHYST(Structure):        # 29/LC  Skalar physical values in all nodes (Multiphysic)
   _fields_ = [
         ('m_time', c_float),      # [  90] Time step
         ('m_type', c_float)       #        Type of the values
      ]
cn_mphyst = CN_MPHYST()
...
class CSLN_SUPP(Structure):        # 39/NR:102  Support of Structural lines
   _fields_ = [
         ...
         ('m_tal', c_float * 3),   #        Vector to be aligned as local z-axis
...
class CSLN_EXYZ(Structure):        # 39/NR:120  Explicit nodes on lines
   _fields_ = [
         ...
         ('m_id1', c_int),         #        reserved
         ('m_xyz', c_float * 3 * 64)  # [1001] XYZ coordinates
      ]
csln_exyz = CSLN_EXYZ()
...
class CSAR_EXYZ(Structure):        # 39/NR:220  Explicit QUAD nodes on area
   _fields_ = [
         ...
         ('m_id1', c_int),         #        reserved
         ('m_xyz', c_float * 3 * 4)  # [1001] XYZ coordinates
      ]
csar_exyz = CSAR_EXYZ()
...
class CGAR_BOUN(Structure):        # 32/NR:1  Outer Boundary of Area
   _fields_ = [
         ...
         ('m_ifc', c_int),         #        Interface hinge bit pattern (Released DOFs)
         ...
         ('m_cx', c_float),        # [1096] bedding stiffness along the boundary edge                      
         ('m_cy', c_float),        # [1096] bedding stiffness on the area's in-plane direction             
         ('m_cz', c_float),        # [1096] bedding stiffness on the area's out-of-plane direction         
         ('m_cmx', c_float),       # [1099] torsional bedding stiffness about the boundary edge            
         ('m_cmy', c_float),       # [1099] torsional bedding stiffness about the area's in-plane direction
         ('m_cmz', c_float)        # [1099] torsional bedding stiffness about the area's out-of-plane direc
      ]
cgar_boun = CGAR_BOUN()
...
class CGVO_SURF(Structure):        # 33/NR:+  Involved Surfaces
   _fields_ = [
         ...
         ('m_phi', c_float),       # [1200] end rotation angle for extrusion/rotation
         ('m_phi0', c_float),      # [1200] start rotation angle for rotation
         ('m_dphi', c_float),      # [1200] allowable sector angle for rotation
...
class CTOWR_DEF(Structure):        # 34/ID:2  Tower definition
   _fields_ = [
         ...
         ('m_box', c_float * 2 * 4), # [1001] Scope box [X:Y,1:4]
...
class CSGRP_BOX(Structure):        # 37/ID:1  Selective Basic-Volume
   _fields_ = [
         ...
         ('m_bit', c_int),         #        option
         ('m_par', c_float * 3 * 7)  # [1001] PARAMETERS OF BOX par(3,0:6)
      ]
csgrp_box = CSGRP_BOX()
...
class CSGRP_POL(Structure):        # 37/ID:2  Selective Polyeder-Volume
   _fields_ = [
         ...
         ('m_bit', c_int),         #        option
         ('m_t', c_float * 3 * 4), #        transformation matrix to local coordinates
         ('m_ext', c_float * 3 * 3), # [1001] global extension, local extensions min/max
         ('m_vert', c_float * 3 * 63)  # [1001] vertices in local coordinates r,s,t
      ]
csgrp_pol = CSGRP_POL()
...
class CGLAR_0(Structure):          # 38/NR:0  Load distribution areas Header
   _fields_ = [
         ...
         ('m_xyz', c_float * 3 * 4)  # [1001] ! 4 corner points of area (optional)
      ]
cglar_0 = CGLAR_0()
...
class CTRB(Structure):             # 1038/NR:0  Tributary areas - Header
   _fields_ = [
         ('m_id', c_int),          #        Identification 0
         ('m_nr', c_int),          #        Id of the tributary area
         ('m_sar', c_int),         #        Id of the referenced structural master area
         ('m_loadfunc', c_int),    #        Loading function:
         ('m_refine', c_int),      #        Number of preceeding refinements
         ('m_m', c_int),           #        Number of subdivisions in local x-direction of user-defined are
         ('m_n', c_int),           #        Number of subdivisions in local y-direction of user-defined are
         ('m_nnodes', c_int),      #        Number of nodes        TRB_NODE 1038/NR:2
         ('m_ntris', c_int),       #        Number of triangles    TRB_TRI  1038/NR:3
         ('m_ccav', c_int),        #        Optimization at concave corners
         ('m_id2', c_int),         #        unused
         ('m_id3', c_int),         #        unused
         ('m_xyz', c_float * 3 * 4)  # [1001] Four points, if user defined area
      ]
ctrb = CTRB()

class CTRB_SUPP(Structure):        # 1038/NR:1  Tributary areas - Supports
   _fields_ = [
         ('m_id', c_int),          #        Identification 1
         ('m_suptype', c_int),     #        Type of support selection
         ('m_supid', c_int),       #        Support id, if SUPTYPE = 2,3,4
         ('m_supbcmod', c_int),    #        Filter for boundary conditions (Cadinp: FFIX)
         ('m_addmode', c_int),     #        Support(s) are either added or ignored (Cadinp: MODE)
         ('m_ftyp', c_int),        #        Filter for support type (Cadinp: FTYP)
         ('m_id2', c_int),         #        unused
         ('m_id3', c_int),         #        unused
         ('m_bboxh', c_float),     #        Search height (in local z-direction) of the oriented bounding b
         ('m_stiff', c_float),     #        Pseudo/Relative stiffness of supports (Cadinp: STIF)
         ('m_angle', c_float),     #        Filter for support selections, only intersection with the given
         ('m_angletol', c_float),  #        Tolerance for ANGLE (Cadinp: FANT)
         ('m_val1', c_float),      #        unused
         ('m_val2', c_float),      #        unused
         ('m_val3', c_float),      #        unused
         ('m_val4', c_float),      #        unused
         ('m_guid', c_int * 32)    #        256 Bit GUId Identifier, if SUPTYPE = 5
      ]
ctrb_supp = CTRB_SUPP()

class CTRB_NODE(Structure):        # 1038/NR:2  Tributary areas - Nodes
   _fields_ = [
         ('m_id', c_int),          #        Identification 2
         ('m_nodeid', c_int),      #        Node id
         ('m_id1', c_int),         #        unused
         ('m_id2', c_int),         #        unused
         ('m_xyz', c_float * 3)    # [1001] Coordinates
      ]
...
class CTRB_TRI(Structure):         # 1038/NR:3  Tributary areas - Triangles
   _fields_ = [
         ('m_id', c_int),          #        Identification 3
         ('m_triid', c_int),       #        Triangle id
         ('m_suptype', c_int),     #        Support type
         ('m_supid', c_int),       #        Support id
         ('m_id1', c_int),         #        unused
         ('m_id2', c_int),         #        unused
         ('m_nodes', c_int * 3)    #        Triangle nodes
      ]
ctrb_tri = CTRB_TRI()
...
class CDC_BEAM(Structure):         # 61/LC:0  DesignCaseDefintionBeam
   _fields_ = [
         ('m_id', c_int),          #        Identification 0
         ('m_task', c_int),        #        Type of design check
         ('m_cs', c_int),          #        Construction stage
         ('m_id4', c_int),         #        free
         ('m_id5', c_int),         #        free
         ('m_id6', c_int),         #        free
         ('m_free1', c_float),     #        free
         ('m_free2', c_float),     #        free
         ('m_free3', c_float),     #        free
         ('m_free4', c_float),     #        free
         ('m_text',  c_int * 17)   #        Designation of Designcase
      ]
cdc_beam = CDC_BEAM()

class CDC_DSLN(Structure):         # 62/LC:0  DesignCaseDefintionDsln
   _fields_ = [
         ('m_id', c_int),          #        Identification 0
         ('m_task', c_int),        #        Type of design check
         ('m_cs', c_int),          #        Construction stage
...
class CDC_BSCT(Structure):         # 64/LC:0  DesignCaseDefintionBsct
   _fields_ = [
         ('m_id', c_int),          #        Identification 0
         ('m_task', c_int),        #        Type of design check
         ('m_cs', c_int),          #        Construction stage
...
class CDC_TRUS(Structure):         # 65/LC:0  DesignCaseDefintionTrus
   _fields_ = [
         ('m_id', c_int),          #        Identification 0
         ('m_task', c_int),        #        Type of design check
         ('m_cs', c_int),          #        Construction stage
...
class CDC_CABL(Structure):         # 66/LC:0  DesignCaseDefintionCabl
   _fields_ = [
         ('m_id', c_int),          #        Identification 0
         ('m_task', c_int),        #        Type of design check
         ('m_cs', c_int),          #        Construction stage
...
class CDC_QUAD(Structure):         # 67/LC:0  DesignCaseDefintionQuad
   _fields_ = [
         ('m_id', c_int),          #        Identification 0
         ('m_task', c_int),        #        Type of design check
         ('m_cs', c_int),          #        Construction stage
...
class CDC_BRIC(Structure):         # 68/LC:0  DesignCaseDefintionBric
   _fields_ = [
         ('m_id', c_int),          #        Identification 0
         ('m_task', c_int),        #        Type of design check
         ('m_cs', c_int),          #        Construction stage
...
class CDC1_BEAM(Structure):        # 61/LC:1  DesignCaseMaterialBeam
   _fields_ = [
         ('m_id', c_int),          #        Identification 1
         ('m_mat', c_int),         #        Material number
         ('m_wrklaw', c_int),      #        Worklaw
         ('m_id3', c_int),         #        free
         ('m_id4', c_int),         #        free
         ('m_id5', c_int),         #        free
         ('m_id6', c_int),         #        free
         ('m_scm', c_float),       #        Used safety factor
         ('m_free2', c_float),     #        free
         ('m_free3', c_float),     #        free
         ('m_free4', c_float)      #        free
      ]
...
class CDC1_DSLN(Structure):        # 62/LC:1  DesignCaseMaterialDsln
   _fields_ = [
         ('m_id', c_int),          #        Identification 1
         ('m_mat', c_int),         #        Material number
         ('m_wrklaw', c_int),      #        Worklaw
...
class CDC1_BSCT(Structure):        # 64/LC:1  DesignCaseMaterialBsct
   _fields_ = [
         ('m_id', c_int),          #        Identification 1
         ('m_mat', c_int),         #        Material number
         ('m_wrklaw', c_int),      #        Worklaw
...
class CDC1_TRUS(Structure):        # 65/LC:1  DesignCaseMaterialTrus
   _fields_ = [
         ('m_id', c_int),          #        Identification 1
         ('m_mat', c_int),         #        Material number
         ('m_wrklaw', c_int),      #        Worklaw
...
class CDC1_CABL(Structure):        # 66/LC:1  DesignCaseMaterialCabl
   _fields_ = [
         ('m_id', c_int),          #        Identification 1
         ('m_mat', c_int),         #        Material number
         ('m_wrklaw', c_int),      #        Worklaw
...
class CDC1_QUAD(Structure):        # 67/LC:1  DesignCaseMaterialQuad
   _fields_ = [
         ('m_id', c_int),          #        Identification 1
         ('m_mat', c_int),         #        Material number
         ('m_wrklaw', c_int),      #        Worklaw
...
class CDC1_BRIC(Structure):        # 68/LC:1  DesignCaseMaterialBric
   _fields_ = [
         ('m_id', c_int),          #        Identification 1
         ('m_mat', c_int),         #        Material number
         ('m_wrklaw', c_int),      #        Worklaw
...
class CSLVL_RES(Structure):        # 87/LC:+  Structural Storey Level Results
   _fields_ = [
         ...
         ('m_ux', c_float),        # [1003] average storey displacement in X (=translations at COM)
...
class CDSLN_SCT(Structure):        # 120/00:0  Design element sections
   _fields_ = [
         ...
         ('m_x', c_float),         # [1001] sectional abscissa
         ('m_t', c_float * 3 * 4), #        transformation matrix
...
class CBEAM_TSN(Structure):        # 104/LC:Z!  thermal eigen stress per material
   _fields_ = [
         ...
         ('m_x', c_float),         # [1001] distance from start
         ('m_ts', c_float * 2 * 32)  # [1092] Eigenstress table (z,sig)
      ]
cbeam_tsn = CBEAM_TSN()
...
class CBEAM_TST(Structure):        # 105/LC:+  thermal eigen stress per material
   _fields_ = [
         ...
         ('m_x', c_float),         # [1001] distance from start
         ('m_ts', c_float * 2 * 128)  # [1092] Eigenstress table (z,sig)
      ]
cbeam_tst = CBEAM_TST()
...
class CBEAM_RFI(Structure):        # 106/DC:+  info of reinforcement of beams
   _fields_ = [
         ...
         ('m_x', c_float),         # [1001] distance from start
         ('m_asl', c_int * 2 * 10),#        infos for longitudinal layers 0:9
         ('m_asv', c_int * 2 * 15) #        infos for transverse reinforcement layers 1:15
      ]
cbeam_rfi = CBEAM_RFI()
...
class CBEAM_RFC(Structure):        # 106/DC:+  reinforcement of beams
   _fields_ = [
         ...
         ('m_asb', c_float * 2 * 15)  # [1021] transverse reinforcements
      ]
cbeam_rfc = CBEAM_RFC()
...
class CDSLN_RFS(Structure):        # 126/DC:Z+  results of shear wall design
   _fields_ = [
         ...
         ('m_asb', c_float * 2 * 15), # [1021] transverse shear (A(1,i)) / confinement (A(2,i)) reinforcement
...
class CBEAM_UC0(Structure):        # 1107/LC:0  Maximum utilisations in cross-section of beams
   _fields_ = [
         ...
         ('m_val1', c_float),      #        free
         ('m_val2', c_float),      #        free
         ('m_eta_my', c_float),    # [1299] Plastic utilisation: My/Mypl (neg. if not applicable, i.e., pla
         ('m_eta_vz', c_float),    # [1299] Plastic utilisation: Vz/Vzpl
         ('m_eta_el', c_float),    # [1299] Elastic utilisation: von Mises, with buckling reduction if elas
         ('m_eta_slsp', c_float),  # [1299] Utilisation for sls,permanent
         ('m_eta_slsr', c_float)   # [1299] Utilisation for sls,rare
      ]
cbeam_uc0 = CBEAM_UC0()
...
class CBEAM_HR0(Structure):        # 111/LC:0  Maximum of Implicit Hinge Reactions
   _fields_ = [
         ('m_id', c_int),          #        identifier 0
         ('m_ibit', c_int * 3),    #        reserved
         ('m_x', c_float),         # [1001] max. beam length
         ('m_reac', c_float),      #        extremal reaction force/ moment
         ('m_stv', c_float * 10),  #        extremal values of statevar's
         ('m_pidx', c_float * 6)   #        extremal values of performance indices by category
      ]
cbeam_hr0 = CBEAM_HR0()
...
class CBEAM_HRC(Structure):        # 111/LC:+  Implicit Hinge Reactions
   _fields_ = [
         ...
         ('m_mtyp', c_int),        #        material type, cf.
         ('m_ibit', c_int),        #        reserved
         ('m_x', c_float),         # [1001] hinge location, beam abscissa value
         ('m_reac', c_float),      #        reaction force/ moment
         ('m_stv', c_float * 11),  #        statevar's
         ('m_pidx', c_float * 6)   #        0:performance index for category PERF_CAT_UNSPEC, 1-5: performa
      ]
cbeam_hrc = CBEAM_HRC()
...
class CBEAM_TF0(Structure):        # 116/LC:0  Maximum of tendon forces in beams
   _fields_ = [
         ('m_id', c_int),          #        identifier 0
         ('m_tndid', c_float),     # [1001] Tendon id
         ('m_free', c_float * 4),  #        reserved
         ('m_x', c_float),         # [1001] max beam/dsln length
         ('m_p', c_float),         # [1101] max tendon force
         ('m_df_duct', c_float),   # [1101] min duct compensation force
         ('m_df_tend', c_float),   # [1101] max tendon compensation force
         ('m_eps_pmt0', c_float),  #        max strain difference between tendon and concrete
         ('m_dy', c_float),        # [1001] y-distance of tendon to el.center
         ('m_dz', c_float),        # [1001] z-distance of tendon to el.center
         ('m_age', c_float),       # [  93] max age since prestress
         ('m_relz', c_float)       #        max accumulated relaxation
      ]
cbeam_tf0 = CBEAM_TF0()

class CBEAM_TF(Structure):         # 116/LC:0  Tendon forces in beams
   _fields_ = [
         ('m_nr', c_int),          #        Beam/dsln id
         ('m_tndid', c_float),     # [1001] Tendon id
         ('m_free', c_float * 4),  #        reserved
         ('m_x', c_float),         # [1001] Distance from start
         ('m_p', c_float),         # [1101] Tendon force
         ('m_df_duct', c_float),   # [1101] Duct compensation force (subtracts A_duct from A_total=gross se
         ('m_df_tend', c_float),   # [1101] Tendon compensation force (subtracts A_p from A_duct)
         ('m_eps_pmt0', c_float),  #        Strain difference between tendon and concrete
         ('m_dy', c_float),        # [1001] y-distance of tendon to el.center (of the gross section)
         ('m_dz', c_float),        # [1001] z-distance of tendon to el.center (of the gross section)
...
class CSPRI_RES(Structure):        # 170/LC:+  results of spring-elements
   _fields_ = [
         ...
         ('m_stvt', c_float * 13)  #        State variable / Damage Parameter for CT
      ]
cspri_res = CSPRI_RES()

class CLINK(Structure):            # 1170/00:+  Link elements
   _fields_ = [
         ('m_nr', c_int),          #        element number
         ('m_node', c_int * 2),    #        node number start/end
         ('m_mno', c_int),         #        obligatory reference to a material
         ('m_dofb', c_int),        #        unused
         ('m_nref', c_int),        #        structural parent object (cf. Rec: 170/00 SPRI)
         ('m_ibt6', c_int),        #        unused
         ('m_dl', c_float),        # [1001] element length
         ('m_t', c_float * 3 * 3), #        transformation matrix
         ('m_aref', c_float),      #        reserved (reference area/ unit-less scaling factor)
         ('m_s_intf', c_float),    #        reserved (interface location, should be implied by constitutive
         ('m_k', c_float * 7)      #        reserved (local stiffness values diagonal entries Kx, Ky, Kz,
      ]
clink = CLINK()

class CLINK_RES(Structure):        # 1170/LC:+  Link Element Reactions
   _fields_ = [
         ('m_nr', c_int),          #        element number
         ('m_typ', c_int),         #        reaction type (direction)
         ('m_mtyp', c_int),        #        material type, cf. Rec: 173/NR:0  SPRI_WL0
         ('m_ibit', c_int),        #        reserved
         ('m_dummy', c_float),     #        reserved
         ('m_reac', c_float),      #        reaction force/ moment
         ('m_disp', c_float),      #        displacement translational/ rotational
         ('m_stv', c_float * 12),  #        statevar's
         ('m_pidx', c_float * 6)   #        0:performance index for category PERF_CAT_UNSPEC, 1-5: performa
      ]
clink_res = CLINK_RES()
...
class CSPRI_WL(Structure):         # 173/NR:+  force-displacement / moment-rotation work law
   _fields_ = [
         ...
         ('m_gapp', c_float),      # [1003] initial slip in positive direction (magnitude), [1004] for SPRI
         ('m_gapn', c_float),      # [1003] initial slip in negative direction (magnitude), [1004] for SPRI
         ('m_fric', c_float),      #        friction coefficient
         ('m_coh', c_float),       # [1089] cohesion
         ('m_alph', c_float),      #        stiffness reduction factor for mod. Takeda model
         ('m_beta', c_float),      #        plasticity weight factor for mod. Takeda model
         ('m_par', c_float),       #        further reserved material parameters
         ('m_arb', c_float * 5 * 20)  #        Function values
      ]
cspri_wl = CSPRI_WL()

class CLINK_FPB(Structure):        # 173/NR:5?  Friction pendulum bearing parameters
   _fields_ = [
         ('m_id', c_int),          #        Identifier
         ('m_ibit', c_int * 3),    #        reserved
         ('m_kx', c_float),        # [1095] Axial elastic stiffness [kN/m]
         ('m_kl', c_float * 3),    # [1095] Isotropic lateral elastic stiffness [kN/m], default: \TODO
         ('m_kls', c_float * 3),   # [1095] Isotropic lateral stopper stiffness [kN/m], default: elastic st
         ('m_fxmax', c_float),     # [1101] Threshold (< 0.0) for max axial force (= least compression),
         ('m_fxref', c_float),     # [1101] Axial compressive force (<0.0) reference level,
         ('m_reff', c_float * 3),  # [1001] Effective radii [m]
         ('m_dlim', c_float * 3),  # [1001] Distances to stopper [m]
         ('m_mu', c_float * 3),    #        Friction coefficients [-]
         ('m_muf', c_float * 3),   #        Friction coefficients, fast movement [-] (unused)
         ('m_eta', c_float * 3)    # [ 194] rate parameter [sec/m]: mu,eff(v) = muf - (muf-mu)*exp(-v*eta) 
      ]
clink_fpb = CLINK_FPB()
...
class CSPRI_WLP(Structure):        # 173/NR:9??  Optional performance limits for force-displacement / moment-rot
   _fields_ = [
         ...
         ('m_plims', c_float * 5 * 20)  #        Performance limits
      ]
cspri_wlp = CSPRI_WLP()
...
class CSPRI_MOV(Structure):        # 174/00  Moving Springs
   _fields_ = [
         ...
         ('m_l0', c_float),        # [1001] L0
         ('m_tolq', c_float),      # [1001] TOLQ
         ('m_rich', c_float),      # [1001] RICH
         ('m_outf', c_float)       # [1001] OUTF
      ]
cspri_mov = CSPRI_MOV()
...
class CQUAD_TEN(Structure):        # 200/5  tendons of Quad-elements
   _fields_ = [
         ...
         ('m_xyzt', c_float * 3 * 2), # [1001] Coordinates of tendon start & endpoint
         ('m_xyzh', c_float * 3 * 2), # [1001] Coordinates of duct start & endpoint
...
class CQUAD_RIM(Structure):        # 200/6  Prescribed Reinforcements of Quad-elements
   _fields_ = [
         ...
         ('m_rio', c_float),       # [1200] direction of upside reinforcement
         ('m_rioq', c_float),      # [1200] direction of upside cross reinforcement
         ('m_rioi', c_float),      # [1200] direction of upside third reinforcement
         ('m_riu', c_float),       # [1200] direction of downside reinforcement
         ('m_riuq', c_float),      # [1200] direction of downside cross reinforcement
         ('m_riui', c_float),      # [1200] direction of downside third reinforcement
...
class CQUAD_RIL(Structure):        # 200/7:+  Reinforcement Layers of Quad-elements
   _fields_ = [
         ...
         ('m_rot', c_float),       # [1200] Angle with reference to the local x-direction
...
class CQCUT_DIR(Structure):        # 200/8:+  Material direction
   _fields_ = [
         ...
         ('m_alf', c_float),       # [1200] warp direction
...
class CQUAD_LOA(Structure):        # 202/LC  Quad-element loads
   _fields_ = [
         ...
         ('m_yref', c_float),      # [1001] 
         ('m_zref', c_float),      # [1001] 
         ('m_tfun', c_float * 2 * 16)  #        optional function of load values across the
      ]
cquad_loa = CQUAD_LOA()
...
class CQUAD_LT(Structure):         # 206/NR  Temperature load profiles for QUADs
   _fields_ = [
         ...
         ('m_temp', c_float * 5 * 20)  #        Temperature values
      ]
cquad_lt = CQUAD_LT()
...
class CQUAD_FOR(Structure):        # 210/LC:+  forces of Quad-elements
   _fields_ = [
         ...
         ('m_ny', c_float),        # [1112] membran force
         ('m_nxy', c_float),       # [1112] membran force
         ('m_sg', c_float * 8 * 4),# [1112] results in GAUSS-points
...
class CQUAD_RFX(Structure):        # 214/LC  additional information for primary loadcases
   _fields_ = [
         ...
         ('m_eps', c_float * 5 * 8), #        strain
         ('m_riss', c_float * 5 * 8)  #        directions of crack
      ]
cquad_rfx = CQUAD_RFX()
...
class CQMEM_MOD(Structure):        # 218/LC:1  (Enhanced) Membrane modes
   _fields_ = [
         ('m_id', c_int),          #        =1 (Enhanced) Membrane modes
         ('m_nr', c_int),          #        element number
         ('m_ansz', c_int),        #        ansatz for (enhanced) modes:
         ('m_rsrvd', c_int),       #        reserved
         ('m_alph1_x', c_float),   # [1003] enhanced mode #1 in x (for QMEM_MOD_ANSZ_EAS4, QEAS_MEM_ANSZ_EA
         ('m_alph1_y', c_float),   # [1003] in y (for QMEM_MOD_ANSZ_EAS4, QEAS_MEM_ANSZ_EAS8)
         ('m_alph2_x', c_float),   # [1003] enhanced mode #2 in x (for QMEM_MOD_ANSZ_EAS4, QEAS_MEM_ANSZ_EA
         ('m_alph2_y', c_float),   # [1003] in y (for QMEM_MOD_ANSZ_EAS4, QEAS_MEM_ANSZ_EAS8)
         ('m_alph3_x', c_float),   # [1003] enhanced mode #3 in x (for QEAS_MEM_ANSZ_EAS8)
         ('m_alph3_y', c_float),   # [1003] in y (for QEAS_MEM_ANSZ_EAS8)
         ('m_alph4_x', c_float),   # [1003] enhanced mode #4 in x (for QEAS_MEM_ANSZ_EAS8)
         ('m_alph4_y', c_float)    # [1003] in y (for QEAS_MEM_ANSZ_EAS8)
      ]
cqmem_mod = CQMEM_MOD()

class CQDRL_MOD(Structure):        # 218/LC:2  (Enhanced) Ansatz for membrane drilling stiffness
   _fields_ = [
         ('m_id', c_int),          #        =2 Membrane drilling stiffness
         ('m_nr', c_int),          #        element number
         ('m_ansz', c_int),        #        ansatz for drilling stiffness:
         ('m_rsrvd', c_int)        #        reserved
      ]
cqdrl_mod = CQDRL_MOD()

class CQPLT_MOD(Structure):        # 218/LC:3  (Enhanced) Ansatz for plate action
   _fields_ = [
         ('m_id', c_int),          #        =3 plate action
         ('m_nr', c_int),          #        element number
         ('m_anszb', c_int),       #        ansatz for plate bending:
         ('m_anszs', c_int),       #        ansatz for plate shear:
         ('m_rsrvd', c_int * 2),   #        reserved
         ('m_beta1_x', c_float),   # [1004] enhanced mode #1 in x (for QPLT_MOD_ANSZB_EAS4)
         ('m_beta1_y', c_float),   # [1004] in y (for QPLT_MOD_ANSZB_EAS4)
         ('m_beta2_x', c_float),   # [1004] enhanced mode #2 in x (for QPLT_MOD_ANSZB_EAS4)
         ('m_beta2_y', c_float)    # [1004] in y (for QPLT_MOD_ANSZB_EAS4)
      ]
cqplt_mod = CQPLT_MOD()
...
class CQUAD_STP(Structure):        # 220/LC:-  Nonlinear QUAD-stress Headder
   _fields_ = [
         ('m_typ', c_int),         #        enum of nonlinear parameters
...
class CQUAD_RT0(Structure):        # 230/LC:0  maximum of tendon stresses
   _fields_ = [
         ...
         ('m_szg1', c_float),      # [1092] tendon stress GP1
         ('m_szg2', c_float),      # [1092] tendon stress GP2
         ('m_zzx1', c_float),      # [1101] tendon force + grout compensation force (post-bond) GP1
         ('m_zzx2', c_float),      # [1101] tendon force + grout compensation force (post-bond) GP2
         ('m_zhg1', c_float),      # [1101] duct compensation force (pre-bond) GP1
         ('m_zhg2', c_float),      # [1101] duct compensation force (pre-bond) GP2
         ('m_zzg1', c_float),      # [1101] tendon force GP1
         ('m_zzg2', c_float),      # [1101] tendon force GP2
         ('m_age', c_float),       # [  93] age since prestress
         ('m_relz', c_float),      #        accumulated relaxation
         ('m_tzg1', c_float),      # [1215] temperature tendon GP1
         ('m_tzg2', c_float),      # [1215] temperature tendon GP2
         ('m_thg1', c_float),      # [1215] temperature tendon duct GP1
         ('m_thg2', c_float)       # [1215] temperature tendon duct GP2
      ]
cquad_rt0 = CQUAD_RT0()
...
class CQUAD_RIC(Structure):        # 260/DC:0  maximum reinforcement in Quad-elements
   _fields_ = [
         ...
         ('m_origin', c_float * 7),#        origins of maximum reinforcements
...
class CBRIC_REC(Structure):        # 360/DC:0  maximum reinforcement in Bric-elements
   _fields_ = [
         ...
         ('m_shzmax', c_float),    #        maximum skew principal tensile stress (SLS uncracked)
         ('m_origin', c_float * 3),#        origin of maximum reinforcement
...
class CSEG_DEF(Structure):         # 900/00  Segmentdefinition
   _fields_ = [
         ...
         ('m_alfa', c_float),      # [1200] Angle of start face to axis (0 = match-cast)
         ('m_alfr', c_float),      # [1200] Angle of end   face to axis (0 = match-cast)
...
class CIL_DATA2(Structure):        # -1/NS:1??????  Influence values for INT > 2
   _fields_ = [
         ...
         ('m_fax', c_float * 2 * 8)  #        correction factors for different axle types
      ]
cil_data2 = CIL_DATA2()
