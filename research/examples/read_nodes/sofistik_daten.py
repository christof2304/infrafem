...
class CTRB_SUPP(Structure):        # 1038/NR:1  Tributary areas - Supports
   _fields_ = [
         ...
         ('m_supbcmod', c_int),    #        Boundary condition mode for support selection
         ('m_addmode', c_int),     #        Support(s) are either added or ignored
         ('m_id1', c_int),         #        unused
         ('m_id2', c_int),         #        unused
         ('m_id3', c_int),         #        unused
         ('m_bboxh', c_float),     #        Search height (in local z-direction) of the oriented bounding b
         ('m_stiff', c_float),     #        Pseudo/Relative stiffness of supports
         ('m_angle', c_float),     #        Filter for support selections, only intersection with the given
         ('m_angletol', c_float),  #        Tolerance for ANGLE
         ('m_val1', c_float),      #        unused
         ('m_val2', c_float),      #        unused
         ('m_val3', c_float),      #        unused
         ('m_val4', c_float),      #        unused
...
class CBEAM_RT0(Structure):        # 130/LC:0  maximum of tendon results
   _fields_ = [
         ('m_nr', c_int),          #        identifier 0
         ('m_nrs', c_int),         #        identifier 0
         ('m_igp', c_int),         #        integration scheme (see below)
         ('m_idum', c_int),        #        reserved
         ('m_sigz', c_float),      # [1092] tendon stress
         ('m_zz', c_float),        # [1101] tendon force
         ('m_zh', c_float),        # [1101] duct compensation force
         ('m_sigzg', c_float * 3), # [1092] tendon stress, Gauss point values
         ('m_zxg', c_float * 3),   # [1101] tendon force + grout compensation force (post-bond), Gauss poin
         ('m_zhg', c_float * 3),   # [1101] duct compensation force (pre-bond), Gauss point values
         ('m_zzg', c_float * 3),   # [1101] tendon force, Gauss point values
         ('m_age', c_float),       # [  93] age since prestress
         ('m_relz', c_float)       #        accumulated relaxation
      ]
cbeam_rt0 = CBEAM_RT0()

class CBEAM_RTS(Structure):        # 130/LC:+  tendon results in integration points
   _fields_ = [
         ('m_nr', c_int),          #        element number
         ('m_nrs', c_int),         #        tendon number
         ('m_igp', c_int),         #        integration scheme
         ('m_idum', c_int),        #        reserved
         ('m_sigz', c_float),      # [1092] tendon stress
         ('m_zz', c_float),        # [1101] tendon force
